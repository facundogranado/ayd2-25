...
int main(int argc, char *argv[]) {
    ...
    printf("Maxima temperatura por año:\n");
    int max_temp[YEARS];
    max_temp_years(table, max_temp);
    for (unsigned int year = 0u; year < YEARS; ++year) {
        printf("Año %d: %d\n", FST_YEAR + year, max_temp[year]);
    }
    printf("Mes con mayor precipitacion por año:\n");
    month_t max_rainfall[YEARS];
    max_rainfall_years(table, max_rainfall);
    for (unsigned int year = 0u; year < YEARS; ++year) {
        printf("Año %d: %u\n", FST_YEAR + year, max_rainfall[year]);
    }
